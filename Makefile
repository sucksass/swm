# Makefile - build, install, and distribution rules for swm

include config.mk

SRC = swm.c drw.c util.c
OBJ = $(SRC:.c=.o)
...
# The current swm.c does not include or otherwise consume config.h, so the
...
swm.o: swm.c config.h drw.h util.h
