...
#define WEWM_MAX_TAGS        31u
#define WEWM_TAG_MASK_CAPACITY WEWM_MAX_TAGS
#define WEWM_DEFAULT_TAG_COUNT WEWM_TAG_MASK_CAPACITY
#define WEWM_TAGMASK_ALL     ((uint32_t)((1u << WEWM_MAX_TAGS) - 1u))
#define WEWM_TITLE_MAX       256u
#define WEWM_STATUS_MAX      256u
#define WEWM_STATUS_FALLBACK "wewm"
#define WEWM_DEFAULT_TITLE   "untitled"
#define WEWM_NO_MONITOR      (-1)
#define WEWM_BAR_PADDING     4u
#ifndef WEWM_CLIENT_BORDER_WIDTH
#define WEWM_CLIENT_BORDER_WIDTH 1u
#endif
#ifndef WEWM_DEFAULT_FLOATING
#define WEWM_DEFAULT_FLOATING 0
#endif
#ifndef WEWM_BAR_VISIBLE
...
#ifndef WEWM_VERSION
#error "WEWM_VERSION must be supplied by the project configuration/build"
#endif
#ifndef WEWM_FONT_COUNT
#define WEWM_FONT_COUNT 1u
#endif
#ifndef WEWM_FONT_PRIMARY
#define WEWM_FONT_PRIMARY "monospace:size=10"
#endif
#if WEWM_FONT_COUNT != 1u
#error "Component 4 currently requires exactly one configured font; multi-font configuration is defined later"
#endif
#ifndef WEWM_COLOR_SCHEME_COUNT
#define WEWM_COLOR_SCHEME_COUNT 1u
#endif
#ifndef WEWM_COLOR_FG
#define WEWM_COLOR_FG "#ffffff"
#endif
#ifndef WEWM_COLOR_BG
#define WEWM_COLOR_BG "#000000"
#endif
#ifndef WEWM_COLOR_BORDER
#define WEWM_COLOR_BORDER "#000000"
#endif
#ifndef WEWM_COLOR_FG_SELECTED
#define WEWM_COLOR_FG_SELECTED WEWM_COLOR_FG
#endif
#ifndef WEWM_COLOR_BG_SELECTED
#define WEWM_COLOR_BG_SELECTED WEWM_COLOR_BG
#endif
#ifndef WEWM_COLOR_BORDER_SELECTED
#define WEWM_COLOR_BORDER_SELECTED WEWM_COLOR_BORDER
#endif

#if WEWM_MAX_TAGS == 0 || WEWM_MAX_TAGS >= 32
#error "WEWM_MAX_TAGS must fit in a 32-bit tag mask"
#endif
...
struct Rule {
    ...
    uint32_t tags;
    bool floating;
    int monitor; /* WEWM_NO_MONITOR means no forced monitor. */
};

/* The configuration layer may replace this empty ordered rule set later. */
static const Rule wewm_rules[1] = { { NULL, NULL, NULL, 0u, false, WEWM_NO_MONITOR } };
static const size_t wewm_rule_count = 0;
...
static char root_status[WEWM_STATUS_MAX];
static const char default_client_title[] = WEWM_DEFAULT_TITLE;
...
static int wewm_runtime_xerror(Display *display, XErrorEvent *event);
static unsigned int wewm_get_numlock_mask(void);

static unsigned int wewm_modifier_variant_count(unsigned int mod,
                                                 unsigned int variants[4]);
static void wewm_grab_keys(void);
static void wewm_regrab_keys(void);
static void wewm_grab_buttons(Window win, enum ClickRegion region);
static void wewm_grab_client_buttons(Client *c, bool focused);
static void wewm_grab_root_buttons(void);
static void wewm_grab_bar_buttons(Monitor *m);

static void wewm_focus(Monitor *m, Client *target);
static void wewm_set_fullscreen(Client *c, bool fullscreen);
static void wewm_send_synthetic_configure(Client *c);
static bool wewm_client_protocol_supported(Client *c, Atom protocol);
static void wewm_send_protocol(Client *c, Atom protocol);
static void wewm_create_bar(Monitor *m);
static void wewm_update_bars(void);
static void wewm_drawbar(Monitor *m);
static void wewm_drawbars(void);
static void wewm_arrange(Monitor *m);
static void wewm_arrange_all(void);
static void wewm_arrange_tiled(Monitor *m);
static void wewm_arrange_monocle(Monitor *m);
static void wewm_dispatch_button_press(XEvent *ev);
static void wewm_dispatch_client_message(XEvent *ev);
static void wewm_dispatch_configure_request(XEvent *ev);
static void wewm_dispatch_configure_notify(XEvent *ev);
static void wewm_dispatch_destroy_notify(XEvent *ev);
static void wewm_dispatch_enter_notify(XEvent *ev);
static void wewm_dispatch_expose(XEvent *ev);
static void wewm_dispatch_focus_in(XEvent *ev);
static void wewm_dispatch_key_press(XEvent *ev);
static void wewm_dispatch_map_request(XEvent *ev);
static void wewm_dispatch_mapping_notify(XEvent *ev);
static void wewm_dispatch_motion_notify(XEvent *ev);
static void wewm_dispatch_property_notify(XEvent *ev);
static void wewm_init_status(void);
static void wewm_unmanage(Client *c, bool destroyed);
static void wewm_action_spawn(const Arg *arg);
static void wewm_action_focus_stack(const Arg *arg);
static void wewm_action_inc_nmaster(const Arg *arg);
static void wewm_action_set_mfact(const Arg *arg);
static void wewm_action_zoom(const Arg *arg);
static void wewm_action_cycle_saved(const Arg *arg);
static void wewm_action_toggle_floating(const Arg *arg);
static void wewm_action_toggle_bar(const Arg *arg);
static void wewm_action_view(const Arg *arg);
static void wewm_action_toggle_view(const Arg *arg);
static void wewm_action_tag(const Arg *arg);
static void wewm_action_toggletag(const Arg *arg);
static void wewm_action_focus_monitor(const Arg *arg);
static void wewm_action_send_monitor(const Arg *arg);
static bool wewm_transfer_client(Client *c, Monitor *to);
static void wewm_action_move_resize(const Arg *arg);
static void wewm_action_kill(const Arg *arg);
static void wewm_action_quit(const Arg *arg);
 ...
 */
static bool
wewm_valid_tag_count(unsigned int count)
{
    return count > 0 && count <= WEWM_TAG_MASK_CAPACITY;
}

static bool
wewm_valid_tag_mask(uint32_t mask)
{
    return mask != 0 && (mask & ~WEWM_TAGMASK_ALL) == 0;
}

static bool
wewm_set_tag_count(unsigned int count)
{
    if (!wewm_valid_tag_count(count))
        return false;
    tag_count = count;
    return true;
}
...
static bool
wewm_set_client_tags(Client *c, uint32_t mask)
{
    if (!c || !wewm_valid_tag_mask(mask))
        return false;
    c->tags = mask;
    return true;
}
...
static bool
wewm_set_visible_tags(Monitor *m, uint32_t mask)
{
    if (!m || !wewm_valid_tag_mask(mask))
        return false;
...
static bool
wewm_monitor_list_circular(const Monitor *head)
{
    const Monitor *slow;
    const Monitor *fast;
...
static bool
wewm_active_monitor_valid(const Monitor *head, const Monitor *active)
{
    const Monitor *m;

    if (!head)
        return active == NULL;
    if (!active || !wewm_monitor_list_circular(head))
        return false;
...
static void
wewm_set_status(const char *text)
{
    size_t n;

    if (!text || !*text)
        text = WEWM_STATUS_FALLBACK;
...
static void
wewm_set_client_title(Client *c, const char *text)
{
    size_t n;
...
static const Layout wewm_layout_tiled = { "+", wewm_arrange_tiled };
static const Layout wewm_layout_monocle = { "[ ]", wewm_arrange_monocle };

static void
wewm_init_monitor(Monitor *m, int index)
{
    if (!m)
        return;
    ...
    memset(m, 0, sizeof(*m));
    m->num = index;
    m->barvisible = WEWM_BAR_VISIBLE != 0;
    m->bartop = WEWM_BAR_TOP != 0;
    m->mfact = 0.55f;
    m->nmaster = 1;
    m->layout_slot = 0;
    m->layout_label[0] = '\0';
    m->lt[0] = &wewm_layout_tiled;
    m->lt[1] = &wewm_layout_monocle;
    m->tagset_slot = 0;
    m->tagset[0] = 1u;
    m->tagset[1] = 1u;
    m->next = m;
...
static bool
wewm_same_geometry(const Geometry *a, const Geometry *b)
{
...
static void
wewm_set_monitor_geometry(Monitor *m, int x, int y,
                           unsigned int w, unsigned int h)
{
    ...
    m->full.w = w;
    m->full.h = h;
    wewm_update_bar_pos(m);
}

static size_t
wewm_monitor_count(void)
{
    const Monitor *m;
    size_t n = 0;
...
static Monitor **
wewm_monitor_array(size_t count)
{
    Monitor **array;
    Monitor *m;
    size_t i = 0;
...
static bool
wewm_monitor_geometry_unique(const Geometry *geoms, size_t count,
                             const Geometry *candidate)
{
    size_t i;

    for (i = 0; i < count; i++) {
        if (wewm_same_geometry(&geoms[i], candidate))
            return false;
    }
    return true;
}
...
static void
wewm_append_client(Client **head, Client *c, bool stack_link)
{
    Client **p;
...
static void
wewm_migrate_monitor_clients(Monitor *from, Monitor *to)
{
    Client *c, *next;
    ...
    for (c = from->clients; c; c = next) {
        next = c->next;
        c->mon = to;
        wewm_append_client(&to->clients, c, false);
    }
    from->clients = NULL;
    ...
    for (c = from->stack; c; c = next) {
        next = c->snext;
        c->mon = to;
        wewm_append_client(&to->stack, c, true);
    }
    from->stack = NULL;
...
static void
wewm_link_monitor_list(Monitor **list, size_t count)
{
    size_t i;
...
static Monitor *
wewm_find_geometry_match(Monitor **old, bool *used, size_t old_count,
                         const Geometry *geometry)
{
    size_t i;

    for (i = 0; i < old_count; i++) {
        if (!used[i] && wewm_same_geometry(&old[i]->full, geometry)) {
            used[i] = true;
            return old[i];
        }
    }
...
static Monitor *
wewm_find_unmatched_old(Monitor **old, bool *used, size_t old_count)
{
    size_t i;
...
static bool
wewm_discover_monitors(void)
{
    Geometry *geoms = NULL;
    (void)wewm_monitor_geometry_unique;
    Monitor **old = NULL;
    Monitor **newlist = NULL;
    bool *used = NULL;
    size_t old_count = wewm_monitor_count();
    ...
    if (XineramaIsActive(dpy)) {
        ...
        if (info && screen_count > 0) {
            ...
            for (i = 0; i < (size_t)screen_count; i++) {
                ...
                g.bw = 0;
                if (g.w && g.h &&
                    wewm_monitor_geometry_unique(geoms, geom_count, &g))
                    geoms[geom_count++] = g;
            }
        }
        if (info)
    ...
    old = wewm_monitor_array(old_count);
    used = ecalloc(old_count ? old_count : 1, sizeof(*used));
    newlist = ecalloc(geom_count, sizeof(*newlist));

    for (i = 0; i < geom_count; i++) {
        Monitor *m = wewm_find_geometry_match(old, used, old_count, &geoms[i]);

        if (!m)
            m = wewm_find_unmatched_old(old, used, old_count);
        if (!m) {
            m = ecalloc(1, sizeof(*m));
            wewm_init_monitor(m, (int)i);
            changed = true;
        }

        if (m->num != (int)i || !wewm_same_geometry(&m->full, &geoms[i]))
            changed = true;

        m->num = (int)i;
        wewm_set_monitor_geometry(m, geoms[i].x, geoms[i].y,
                                  geoms[i].w, geoms[i].h);
        newlist[i] = m;
    }
    ...
    for (i = 0; i < old_count; i++) {
        if (!used[i]) {
            ...
            if (survivor)
                wewm_migrate_monitor_clients(old[i], survivor);
            if (oldsel == old[i])
                oldsel = survivor;
    ...
    wewm_link_monitor_list(newlist, geom_count);
    selmon = oldsel;
    if (!wewm_active_monitor_valid(mons, selmon))
        selmon = mons;

    if (mons && !wewm_monitor_list_circular(mons))
        die("wewm: monitor list is not circular after discovery");
    if (!wewm_active_monitor_valid(mons, selmon))
        die("wewm: invalid active monitor after discovery");
...
static Monitor *
wewm_monitor_at_rect(int x, int y, unsigned int w, unsigned int h)
{
    Monitor *m, *best = NULL;
...
static Monitor *
wewm_monitor_at_window(Window win)
{
    Monitor *m;
    Client *c;
    Window child;
    ...
    if (win == root) {
        if (XQueryPointer(dpy, root, &child, &child, &root_x, &root_y,
                          &x, &y, &mask))
            return wewm_monitor_at_rect((int)root_x, (int)root_y, 1, 1);
        return selmon;
    }
...
static bool
wewm_get_window_state(Window win, long *state)
{
    Atom actual;
    int format;
...
static bool
wewm_window_is_iconic(Window win)
{
    long state;

    return wewm_get_window_state(win, &state) && state == IconicState;
}

static void
wewm_set_window_state(Window win, long state)
{
...
static bool
wewm_get_net_wm_name(Window win, char *out, size_t out_size)
{
    Atom actual;
    int format;
    unsigned long nitems, bytes_after;
...
static void
wewm_update_client_title(Client *c)
{
    XTextProperty prop;
    char **list = NULL;
    int count = 0;
    ...
    if (wewm_get_net_wm_name(c->win, title, sizeof(title))) {
        wewm_set_client_title(c, title);
        return;
    }
    ...
    if (XGetWMName(dpy, c->win, &prop) && prop.value && prop.nitems) {
        if (XmbTextPropertyToTextList(dpy, &prop, &list, &count) >= Success &&
            ...
            wewm_set_client_title(c, list[0]);
    ...
    if (prop.value)
        XFree(prop.value);
    wewm_set_client_title(c, NULL);
}

static void
wewm_update_size_hints(Client *c)
{
    XSizeHints size;
    long supplied = 0;
    bool have = false;
...
static void
wewm_update_window_state(Client *c)
{
    Atom actual;
    int format;
...
static Monitor *
wewm_transient_monitor(Window win, uint32_t *tags, bool *is_transient)
{
    Window parent = None;
    ...
    if (is_transient)
        *is_transient = true;
    parent_client = wewm_wintoclient(parent);
    if (!parent_client)
        return selmon;
    if (tags)
...
static void
wewm_clamp_client_position(Client *c)
{
...
static void
wewm_remove_client_link(Client **head, Client *c, bool stack_link)
{
    Client **p;
...
static bool
wewm_rule_matches(const Rule *rule, const char *class_name,
...
static Monitor *
wewm_monitor_by_index(int index)
{
    Monitor *m;
...
static bool
wewm_client_is_linked(const Client *c)
{
    Monitor *m;
    Client *it;
...
wewm_apply_rules(Client *c)
{
    ...
    if (!c || !c->mon)
        return;
    if (wewm_client_is_linked(c))
        die("wewm: attempted to apply initial client rules after insertion");
    ...
    for (i = 0; i < wewm_rule_count; i++) {
        const Rule *rule = &wewm_rules[i];
        Monitor *preferred;

        if (!wewm_rule_matches(rule, class_name, instance, title))
            continue;

        matched_rule = true;
        rule_tags |= rule->tags & WEWM_TAGMASK_ALL;
        rule_floating = rule->floating;
        if (rule->monitor != WEWM_NO_MONITOR) {
            preferred = wewm_monitor_by_index(rule->monitor);
            if (preferred)
                c->mon = preferred;
        }
    }
     ...
     */
    {
        uint32_t final_tags = (inherited_tags | rule_tags) & WEWM_TAGMASK_ALL;

        if (final_tags != 0u && wewm_valid_tag_mask(final_tags)) {
            if (!wewm_set_client_tags(c, final_tags))
                die("wewm: failed to apply client classification tags");
        } else if (c->mon &&
                   wewm_valid_tag_mask(
                       c->mon->tagset[c->mon->tagset_slot & 1u])) {
            if (!wewm_set_client_tags(
                    c, c->mon->tagset[c->mon->tagset_slot & 1u]))
                die("wewm: failed to assign client visible tags");
        } else {
            die("wewm: client classification produced invalid tags");
        }
    }
    ...
    if (!wewm_active_monitor_valid(mons, c->mon) ||
        !wewm_valid_tag_mask(c->tags))
        die("wewm: invalid client state after rule classification");
}

static void
wewm_manage(Client *c)
{
    if (!c || !c->mon)
        return;

    c->geom.bw = WEWM_CLIENT_BORDER_WIDTH;
    wewm_clamp_client_position(c);
    ...
    wewm_grab_client_buttons(c, false);
    wewm_insert_client(c);
    wewm_update_client_list();
    wewm_set_window_state(c->win, NormalState);
    if (c->fullscreen) {
    ...
    wewm_send_synthetic_configure(c);
    XMapWindow(dpy, c->win);
}

static Client *
wewm_create_client(Window win, XWindowAttributes *wa)
{
    Client *c;
    Monitor *monitor;
    ...
    c->restore.valid = true;
    c->floating = WEWM_DEFAULT_FLOATING != 0;
    c->was_floating = c->floating;
    c->tags = 1u;

    monitor = wewm_transient_monitor(win, &inherited_tags, &transient);
    if (!monitor)
        monitor = selmon;
    if (!monitor || !wewm_active_monitor_valid(mons, monitor)) {
        free(c);
        return NULL;
    }
    c->mon = monitor;
    ...
    wewm_update_client_title(c);
    wewm_update_size_hints(c);
    wewm_update_wm_hints(c);
    wewm_update_window_state(c);
    if (c->fixed || transient)
        c->floating = true;
    c->was_floating = c->floating;

    if (!wewm_valid_tag_mask(c->tags)) {
        free(c);
        return NULL;
    }
    return c;
...
static bool
wewm_manage_window(Window win, bool map_window)
{
    XWindowAttributes wa;
    Client *c;

    if (win == None || win == root || win == wmcheckwin || wewm_wintoclient(win))
        return false;
    if (!XGetWindowAttributes(dpy, win, &wa))
        return false;
    if (wa.override_redirect)
        return false;
    c = wewm_create_client(win, &wa);
    if (!c)
        return false;
    wewm_apply_rules(c);
    wewm_manage(c);
...
static bool
wewm_window_eligible(Window win, bool *transient)
{
    XWindowAttributes wa;
    Window parent = None;
    ...
    return wa.map_state == IsViewable || wewm_window_is_iconic(win);
}

static void
wewm_scan_existing_windows(void)
{
    ...
    for (i = 0; i < nchildren; i++) {
        bool transient;
        if (wewm_window_eligible(children[i], &transient)) {
            transients[i] = transient;
            if (!transient)
                (void)wewm_manage_window(children[i], true);
        }
    }
    ...
    for (i = 0; i < nchildren; i++) {
        ...
        if (!XGetWindowAttributes(dpy, children[i], &wa) || wa.override_redirect)
            continue;
        if (!(wa.map_state == IsViewable || wewm_window_is_iconic(children[i])))
            continue;
...
static void
wewm_unmanage(Client *c, bool destroyed)
{
    Monitor *m;
    bool was_selected;
    ...
    if (m) {
        wewm_remove_client_link(&m->clients, c, false);
        wewm_remove_client_link(&m->stack, c, true);
        if (was_selected)
            m->sel = NULL;
    ...
    old_handler = XSetErrorHandler(wewm_runtime_xerror);
    if (!destroyed) {
        XSetWindowBorderWidth(dpy, c->win, c->restore.geometry.bw);
        XSelectInput(dpy, c->win, NoEventMask);
        wewm_set_window_state(c->win, WithdrawnState);
...
/* Configuration-owned, ordered binding tables.  The default configuration is
 * deliberately empty; a project configuration may replace these definitions. */
static const Key wewm_keys[1] = { { 0u, NoSymbol, NULL, { 0 } } };
static const size_t wewm_key_count = 0;

static const Button wewm_buttons[1] = {
    { ClickRootBackground, 0u, 0u, NULL, { 0 } }
};
static const size_t wewm_button_count = 0;

/* -------------------------- focus / protocols ------------------------- */

static bool
wewm_client_visible(const Client *c, const Monitor *m)
{
    return c && m && c->mon == m &&
           wewm_valid_tag_mask(c->tags) &&
...
static Client *
wewm_focus_candidate(Monitor *m, Client *target)
{
    Client *c;

    if (!m)
        return NULL;
    if (target && wewm_client_visible(target, m))
        return target;
    for (c = m->stack; c; c = c->snext)
        if (wewm_client_visible(c, m))
            return c;
    return NULL;
}

static void
wewm_focus(Monitor *m, Client *target)
{
    Client *old, *newsel;

    if (!m)
        return;
    newsel = wewm_focus_candidate(m, target);
    old = m->sel;

    if (old && old != newsel) {
        wewm_grab_client_buttons(old, false);
        XSetWindowBorder(dpy, old->win, scheme_normal[2].pixel);
    }

    if (newsel) {
        if (newsel != old) {
            wewm_remove_client_link(&m->stack, newsel, true);
            newsel->snext = m->stack;
            m->stack = newsel;
        }
        m->sel = newsel;
        XSetWindowBorder(dpy, newsel->win, scheme_selected[2].pixel);
        wewm_grab_client_buttons(newsel, true);
        if (!newsel->nofocus)
        ...
        if (wewm_client_protocol_supported(newsel,
                                           atoms.slot[AtomWMTakeFocus]))
            wewm_send_protocol(newsel, atoms.slot[AtomWMTakeFocus]);
    } else {
        m->sel = NULL;
...
static bool
wewm_client_protocol_supported(Client *c, Atom protocol)
{
    Atom *list = NULL;
    int count = 0;
    bool found = false;
...
static void
wewm_send_protocol(Client *c, Atom protocol)
{
    XEvent ev;
...
static void
wewm_send_synthetic_configure(Client *c)
{
    XEvent ev;
...
static void
wewm_set_fullscreen_property(Client *c, bool enabled)
{
    Atom actual;
    int format;
...
static void
wewm_set_fullscreen(Client *c, bool fullscreen)
{
    Monitor *m;
    ...
    if (fullscreen) {
        ...
        c->floating = true;
        c->fullscreen = true;
        wewm_set_fullscreen_property(c, true);
        c->geom.x = m->full.x;
        c->geom.y = m->full.y;
        c->geom.w = m->full.w;
        c->geom.h = m->full.h;
        ...
        wewm_set_fullscreen_property(c, false);
        c->fullscreen = false;
...
static Monitor *
wewm_monitor_from_bar(Window win)
{
    Monitor *m;
    if (!mons || win == None)
        return NULL;
...
wewm_create_bar(Monitor *m)
{
    ...
    m->barwin = XCreateWindow(dpy, root, m->full.x, m->by,
                              ...
                              CWBackPixel | CWEventMask | CWCursor, &wa);
    if (m->barwin == None)
        return;
    XDefineCursor(dpy, m->barwin, cursor_normal->cursor);
    wewm_grab_bar_buttons(m);
    if (m->barvisible)
        XMapWindow(dpy, m->barwin);
}

static void
wewm_update_bars(void)
{
    Monitor *m;
...
static bool
wewm_any_urgent_on_tag(unsigned int tag)
{
    Monitor *m;
    Client *c;
    uint32_t bit = 1u << tag;
...
static void
wewm_drawbar(Monitor *m)
{
    unsigned int x = 0, i, statusw = 0, titlew;
    char label[32];
    const char *layout = "";
    const char *title = "";
    ...
    for (i = 0; i < tag_count; i++) {
        ...
        if (w == 0)
            break;
        {
            bool urgent = wewm_any_urgent_on_tag(i);
    ...
    titlew = m->full.w > x + statusw ? m->full.w - x - statusw : 0;
    if (titlew) {
        if (m->sel)
            title = m->sel->title;
        drw_setscheme(drw, m->sel ? scheme_selected : scheme_normal);
        drw_rect(drw, x, 0, titlew, bh, true, false);
        if (m->sel) {
            const char *display_title = title;
            char fixed_title[WEWM_TITLE_MAX + 4];
            if (m->sel->fixed) {
                snprintf(fixed_title, sizeof(fixed_title), "* %s", title);
                display_title = fixed_title;
            }
            drw_text(drw, x, 0, titlew, bh, textpad / 2u,
                     display_title, false);
        }
    }
...
static void
wewm_constrain_size(Client *c, unsigned int *w, unsigned int *h,
                    bool respect_hints)
{
...
static void
wewm_clamp_geometry_to(Monitor *m, Geometry *g, bool full_area)
{
...
static void
wewm_apply_geometry(Client *c, int x, int y, unsigned int w,
                    unsigned int h, bool tiled)
{
    ...
    if (h < 1u) h = 1u;
    if (!tiled)
        wewm_constrain_size(c, &w, &h, true);
    c->geom.x = x;
    c->geom.y = y;
    c->geom.w = w;
    c->geom.h = h;
...
wewm_arrange_tiled(Monitor *m)
{
    ...
    for (c = m->clients; c; c = c->next)
        if (!c->floating && !c->fullscreen && wewm_client_visible(c, m))
            n++;
    if (!n)
        return;
    ...
    for (c = m->clients; c; c = c->next) {
        ...
        if (placed < master_n) {
            ...
            wewm_apply_geometry(c, x, m->work.y + (int)my, w, h, true);
            my += h + 2u * c->geom.bw;
        } else {
            ...
            if (w > 2u * c->geom.bw) w -= 2u * c->geom.bw; else w = 1u;
            if (h > 2u * c->geom.bw) h -= 2u * c->geom.bw; else h = 1u;
            wewm_apply_geometry(c, x, m->work.y + (int)sy, w, h, true);
            sy += h + 2u * c->geom.bw;
        }
        placed++;
    }
...
static void
wewm_arrange_monocle(Monitor *m)
{
    Client *c;
    unsigned int n = 0;
    ...
    for (c = m->clients; c; c = c->next) {
        ...
        wewm_apply_geometry(c, m->work.x, m->work.y, w, h, true);
    }
    (void)n;
}

static void
wewm_arrange(Monitor *m)
{
    Client *c;
    ...
    /* Hide in reverse focus-history order. */
    if (m->stack) {
        Client *rev[256];
        unsigned int n = 0;
        for (c = m->stack; c && n < 256u; c = c->snext)
            rev[n++] = c;
        while (n) {
            c = rev[--n];
            if (!wewm_client_visible(c, m) && c->win != None)
                XMoveWindow(dpy, c->win, INT_MAX / 8, INT_MAX / 8);
        }
    }

    /* Show in focus-history order before final restacking. */
    for (c = m->stack; c; c = c->snext) {
        if (!wewm_client_visible(c, m) || c->win == None)
            continue;
        if (c->fullscreen) {
    ...
    if (m->lt[m->layout_slot & 1u] &&
        m->lt[m->layout_slot & 1u]->arrange == wewm_arrange_monocle) {
    ...
    for (c = m->stack; c && nstack < 256u; c = c->snext)
        if (wewm_client_visible(c, m) && !c->floating && !c->fullscreen)
    ...
    for (c = m->stack; c; c = c->snext)
        if (wewm_client_visible(c, m) && c->floating && !c->fullscreen)
            XRaiseWindow(dpy, c->win);
    if (m->sel && wewm_client_visible(m->sel, m) &&
...
static void
wewm_action_spawn(const Arg *arg)
{
    pid_t pid;
    char *const *argv;
    ...
    if (pid == 0) {
        ...
        execvp(argv[0], argv);
        fprintf(stderr, "wewm: execvp failed: %s\n", strerror(errno));
        _exit(127);
    }
}

static void
wewm_action_focus_stack(const Arg *arg)
{
    Monitor *m = selmon;
    ...
    if (direction > 0) {
        ...
        for (c = first; c; c = c->next)
            if (wewm_client_visible(c, m)) { target = c; break; }
        if (!target)
        ...
        for (c = m->clients; c; c = c->next) {
            if (c == m->sel) break;
            if (wewm_client_visible(c, m)) prev = c;
        }
        if (prev) target = prev;
        else {
            for (c = m->clients; c; c = c->next)
                if (wewm_client_visible(c, m)) target = c;
        }
    }
    if (target)
        wewm_focus(m, target);
...
static void
wewm_action_inc_nmaster(const Arg *arg)
{
    int next;
...
static void
wewm_action_set_mfact(const Arg *arg)
{
    double next;
    if (!selmon || !arg) return;
...
static void
wewm_action_zoom(const Arg *arg)
{
    ...
    for (c = selmon->clients; c; c = c->next)
        if (!c->floating && !c->fullscreen && wewm_client_visible(c, selmon)) {
            if (!target) target = c;
    ...
    if (!target) return;
    wewm_remove_client_link(&selmon->clients, target, false);
    target->next = selmon->clients;
    selmon->clients = target;
    wewm_focus(selmon, target);
    wewm_arrange(selmon);
}

static void
wewm_action_cycle_saved(const Arg *arg)
{
    (void)arg;
    if (!selmon) return;
    selmon->tagset_slot ^= 1u;
    selmon->layout_slot ^= 1u;
    wewm_focus(selmon, NULL);
    wewm_arrange(selmon);
}

static void
wewm_action_toggle_floating(const Arg *arg)
{
    (void)arg;
...
static void
wewm_action_toggle_bar(const Arg *arg)
{
    (void)arg;
    if (!selmon) return;
    selmon->barvisible = !selmon->barvisible;
    wewm_update_bar_pos(selmon);
...
static void
wewm_action_view(const Arg *arg)
{
    uint32_t mask;
    if (!selmon || !arg) return;
    mask = arg->ui & WEWM_TAGMASK_ALL;
    if (!wewm_valid_tag_mask(mask) || !wewm_set_visible_tags(selmon, mask)) return;
    wewm_focus(selmon, NULL);
    wewm_arrange(selmon);
}

static void
wewm_action_toggle_view(const Arg *arg)
{
    uint32_t mask, result;
    if (!selmon || !arg) return;
    mask = arg->ui & WEWM_TAGMASK_ALL;
    result = selmon->tagset[selmon->tagset_slot & 1u] ^ mask;
    if (!wewm_valid_tag_mask(result) || !wewm_set_visible_tags(selmon, result)) return;
    wewm_focus(selmon, NULL);
    wewm_arrange(selmon);
}

static void
wewm_action_tag(const Arg *arg)
{
    if (!selmon || !selmon->sel || !arg) return;
    if (!wewm_set_client_tags(selmon->sel, arg->ui & WEWM_TAGMASK_ALL)) return;
    wewm_focus(selmon, NULL);
    wewm_arrange(selmon);
}

static void
wewm_action_toggletag(const Arg *arg)
{
    uint32_t result;
    if (!selmon || !selmon->sel || !arg) return;
    result = selmon->sel->tags ^ (arg->ui & WEWM_TAGMASK_ALL);
    if (!wewm_valid_tag_mask(result) || !wewm_set_client_tags(selmon->sel, result)) return;
    wewm_focus(selmon, NULL);
    wewm_arrange(selmon);
}

static void
wewm_action_focus_monitor(const Arg *arg)
{
    Monitor *old, *newm;
    int direction;
    ...
    wewm_focus(old, NULL);
    selmon = newm;
    wewm_focus(selmon, selmon->sel);
}

static bool
wewm_transfer_client(Client *c, Monitor *to)
{
    Monitor *from;
    bool was_sel;
    ...
    if (!c || !to || !(from = c->mon) || from == to)
        return false;
    was_sel = from->sel == c;
    wewm_remove_client_link(&from->clients, c, false);
    wewm_remove_client_link(&from->stack, c, true);
    if (was_sel)
        from->sel = NULL;
    c->mon = to;
    if (!wewm_set_client_tags(c, to->tagset[to->tagset_slot & 1u]))
        return false;
    wewm_insert_client(c);
    if (c->fullscreen) {
        c->geom.x = to->full.x;
...
static void
wewm_action_send_monitor(const Arg *arg)
{
    Monitor *from, *to;
    Client *c;
    int direction;
    ...
    if (!wewm_transfer_client(c, to))
        return;
    selmon = to;
    wewm_focus(from, NULL);
    wewm_focus(to, c);
    wewm_arrange(from);
    wewm_arrange(to);
}
...
static void
wewm_action_move_resize(const Arg *arg)
{
    Client *c;
    XEvent ev;
    Cursor cursor;
    ...
    while (XMaskEvent(dpy, PointerMotionMask | ButtonReleaseMask | ExposureMask |
        ...
        if (ev.type == MotionNotify) {
            ...
            if (!resize) {
                ...
                wewm_clamp_geometry_to(c->mon, &c->geom, true);
                ...
                wewm_constrain_size(c, &nw, &nh, true);
                c->geom.w = nw; c->geom.h = nh;
                XMoveResizeWindow(dpy, c->win, c->geom.x, c->geom.y, nw, nh);
            }
        } else if (ev.type == Expose) {
            wewm_dispatch_expose(&ev);
        } else if (ev.type == ConfigureRequest) {
            wewm_dispatch_configure_request(&ev);
    ...
    }
    {
        ...
        Monitor *dest = wewm_monitor_at_rect(c->geom.x, c->geom.y,
                                              c->geom.w, c->geom.h);
        if (dest && dest != origin && wewm_transfer_client(c, dest)) {
            selmon = dest;
            wewm_focus(origin, NULL);
            wewm_focus(dest, c);
            wewm_arrange(origin);
            wewm_arrange(dest);
        } else {
...
static void
wewm_action_kill(const Arg *arg)
{
    int (*old_handler)(Display *, XErrorEvent *);
    (void)arg;
    if (!selmon || !selmon->sel) return;
    if (wewm_client_protocol_supported(selmon->sel,
                                       atoms.slot[AtomWMDeleteWindow])) {
        wewm_send_protocol(selmon->sel, atoms.slot[AtomWMDeleteWindow]);
        return;
    }
    old_handler = XSetErrorHandler(wewm_runtime_xerror);
...
static void
wewm_action_quit(const Arg *arg)
{
    (void)arg;
    running = false;
}
...
static unsigned int
wewm_clean_modifiers(unsigned int state)
{
    return state & 0xffu & ~(unsigned int)LockMask & ~numlockmask;
}

static bool
wewm_button_matches(const Button *button, enum ClickRegion region,
                    unsigned int state, unsigned int detail)
{
    return button && button->region == region && button->button == detail &&
           wewm_clean_modifiers(state) == wewm_clean_modifiers(button->mod);
}

static enum ClickRegion
wewm_bar_region(Monitor *m, int x, unsigned int *tag_index)
{
...
static void
wewm_dispatch_button_press(XEvent *ev)
{
    ...
    m = wewm_monitor_at_window(e->window);
    if (!m) return;
    if (m != selmon) {
        if (selmon) wewm_focus(selmon, NULL);
        selmon = m;
        wewm_focus(selmon, selmon->sel);
    }
    c = wewm_wintoclient(e->window);
    if (c) {
        ...
        region = wewm_bar_region(m, e->x, &tag);
    } else if (e->window == root) {
        region = ClickRootBackground;
    } else {
        return;
    }
    for (i = 0; i < wewm_button_count; i++) {
        const Button *b = &wewm_buttons[i];
        Arg arg;
        if (!wewm_button_matches(b, region, e->state, e->button)) continue;
        arg = b->arg;
...
static void
wewm_dispatch_client_message(XEvent *ev)
{
    XClientMessageEvent *e = &ev->xclient;
    Client *c = wewm_wintoclient(e->window);
    if (e->message_type == atoms.slot[AtomNetWMState] && c) {
        ...
        if (a == atoms.slot[AtomNetWMStateFullscreen] ||
            ...
            wewm_set_fullscreen(c, requested);
        }
...
static void
wewm_dispatch_configure_request(XEvent *ev)
{
    ...
    if (c->floating || !c->mon->lt[c->mon->layout_slot & 1u] ||
        ...
        wewm_constrain_size(c, &c->geom.w, &c->geom.h, true);
        wewm_clamp_geometry_to(c->mon, &c->geom, false);
        if (wewm_client_visible(c, c->mon)) {
            if (mask & CWBorderWidth)
                XSetWindowBorderWidth(dpy, c->win, c->geom.bw);
            XMoveResizeWindow(dpy, c->win, c->geom.x, c->geom.y,
                               c->geom.w, c->geom.h);
...
static void
wewm_dispatch_configure_notify(XEvent *ev)
{
    ...
    if ((unsigned int)e->width == sw && (unsigned int)e->height == sh) {
        changed = wewm_discover_monitors();
    } else {
...
static void
wewm_dispatch_destroy_notify(XEvent *ev)
{
    Client *c = wewm_wintoclient(ev->xdestroywindow.window);
    if (c) wewm_unmanage(c, true);
}

static void
wewm_dispatch_enter_notify(XEvent *ev)
{
...
static void
wewm_dispatch_expose(XEvent *ev)
{
    Monitor *m = wewm_monitor_from_bar(ev->xexpose.window);
    if (m && ev->xexpose.count == 0)
        wewm_drawbar(m);
}

static void
wewm_dispatch_focus_in(XEvent *ev)
{
    Monitor *m;
    Client *c;
    if (ev->xfocus.mode != NotifyNormal && ev->xfocus.mode != NotifyWhileGrabbed)
        return;
    c = wewm_wintoclient(ev->xfocus.window);
    m = c ? c->mon : selmon;
...
static void
wewm_dispatch_key_press(XEvent *ev)
{
    KeySym sym = XLookupKeysym(&ev->xkey, 0);
    unsigned int state = wewm_clean_modifiers(ev->xkey.state);
    size_t i;
    for (i = 0; i < wewm_key_count; i++) {
        const Key *k = &wewm_keys[i];
        if (k->keysym == sym && wewm_clean_modifiers(k->mod) == state) {
...
static void
wewm_dispatch_map_request(XEvent *ev)
{
    XWindowAttributes wa;
    Client *c;
    if (!XGetWindowAttributes(dpy, ev->xmaprequest.window, &wa) || wa.override_redirect)
        return;
    if (wewm_wintoclient(ev->xmaprequest.window)) {
        c = wewm_wintoclient(ev->xmaprequest.window);
        XMapWindow(dpy, c->win);
        return;
    }
    if (wewm_manage_window(ev->xmaprequest.window, true)) {
        c = wewm_wintoclient(ev->xmaprequest.window);
        if (c) {
            if (c->mon == selmon) wewm_focus(c->mon, c);
            wewm_arrange(c->mon);
        }
    }
}
...
static void
wewm_dispatch_mapping_notify(XEvent *ev)
{
    if (ev->xmapping.request == MappingKeyboard ||
        ev->xmapping.request == MappingModifier) {
        XRefreshKeyboardMapping(&ev->xmapping);
        numlockmask = wewm_get_numlock_mask();
        wewm_regrab_keys();
    }
}

static void
wewm_dispatch_motion_notify(XEvent *ev)
{
    Monitor *m;
    if (ev->xmotion.window != root) return;
    m = wewm_monitor_at_rect(ev->xmotion.x_root, ev->xmotion.y_root, 1, 1);
    if (m && m != selmon) {
        if (selmon) wewm_focus(selmon, NULL);
        selmon = m;
        wewm_focus(selmon, selmon->sel);
...
static void
wewm_dispatch_property_notify(XEvent *ev)
{
    ...
    if (e->atom == XA_WM_NORMAL_HINTS) {
        ...
        wewm_update_client_title(c);
        if (c == c->mon->sel) wewm_drawbar(c->mon);
        ...
        if (c->fullscreen != was_fullscreen)
            wewm_set_fullscreen(c, c->fullscreen);
        wewm_arrange(c->mon);
    }
}

static void
wewm_populate_dispatch(void)
{
    memset(&event_dispatch, 0, sizeof(event_dispatch));
    event_dispatch.handlers[ButtonPress] = wewm_dispatch_button_press;
    event_dispatch.handlers[ClientMessage] = wewm_dispatch_client_message;
    event_dispatch.handlers[ConfigureRequest] = wewm_dispatch_configure_request;
    event_dispatch.handlers[ConfigureNotify] = wewm_dispatch_configure_notify;
    event_dispatch.handlers[DestroyNotify] = wewm_dispatch_destroy_notify;
    event_dispatch.handlers[EnterNotify] = wewm_dispatch_enter_notify;
    event_dispatch.handlers[Expose] = wewm_dispatch_expose;
    event_dispatch.handlers[FocusIn] = wewm_dispatch_focus_in;
    event_dispatch.handlers[KeyPress] = wewm_dispatch_key_press;
    event_dispatch.handlers[MapRequest] = wewm_dispatch_map_request;
    event_dispatch.handlers[MappingNotify] = wewm_dispatch_mapping_notify;
    event_dispatch.handlers[MotionNotify] = wewm_dispatch_motion_notify;
    event_dispatch.handlers[PropertyNotify] = wewm_dispatch_property_notify;
}

static int
wewm_startup_xerror(Display *display, XErrorEvent *event)
{
    char text[128];
    ...
    die("wewm: another window manager is already managing the root window%s%s",
        text[0] ? ": " : "", text[0] ? text : "");
    return 0;
}

static int
wewm_runtime_xerror(Display *display, XErrorEvent *event)
{
    char text[128];

    if (event) {
        XGetErrorText(display, event->error_code, text, sizeof(text));
        fprintf(stderr, "wewm: X error: %s\n", text);
    }
    return 0;
}

static void
wewm_check_locale(void)
{
    if (!setlocale(LC_CTYPE, "")) {
        fprintf(stderr, "wewm: warning: could not initialize the system locale\n");
        return;
    }
    if (!XSupportsLocale())
        fprintf(stderr, "wewm: warning: X11 does not support the current locale\n");
}

static void
wewm_check_args(int argc, char **argv)
{
    if (argc == 1)
        return;
    if (argc == 2 && argv[1] &&
    ...
    die("usage: %s [-v|--version]", argv[0] ? argv[0] : "wewm");
}

static void
wewm_claim_root(void)
{
    startup_xerror_seen = false;
    previous_xerror = XSetErrorHandler(wewm_startup_xerror);
    XSelectInput(dpy, root, SubstructureRedirectMask);
    XSync(dpy, False);
    if (startup_xerror_seen)
        die("wewm: failed to acquire root-window management ownership");

    XSetErrorHandler(wewm_runtime_xerror);
    XSync(dpy, False);
}

static void
wewm_init_atoms(void)
{
    ...
    for (i = 0; i < AtomCount; i++) {
        ...
        if (atoms.slot[i] == None)
            die("wewm: failed to intern required X atom");
    }
}

static void
wewm_init_drawing(void)
{
    static const char *fonts[WEWM_FONT_COUNT > 0 ? WEWM_FONT_COUNT : 1] = {
        WEWM_FONT_PRIMARY
    };
    Fnt *primary;
    ...
    if (!drw)
        die("wewm: failed to create drawing context");

    if (WEWM_FONT_COUNT == 0)
        die("wewm: no configured fonts");
    if (!drw_fontset_create(drw, fonts, WEWM_FONT_COUNT))
        die("wewm: no configured font could be loaded");

    primary = drw->fonts;
    if (!primary)
        die("wewm: drawing context has no active font");

    lrpad = primary->h;
    textpad = lrpad;
    bh = primary->h + WEWM_BAR_PADDING;
}

static void
wewm_init_cursors(void)
{
    ...
    if (!cursor_normal || !cursor_resize || !cursor_move)
        die("wewm: failed to create cursor resources");
}

static void
wewm_init_schemes(void)
{
    static const char *normal[3] = {
        WEWM_COLOR_FG, WEWM_COLOR_BG, WEWM_COLOR_BORDER
    };
    static const char *selected[3] = {
        WEWM_COLOR_FG_SELECTED, WEWM_COLOR_BG_SELECTED,
    ...
    if (!scheme_normal || !scheme_selected)
        die("wewm: failed to create color schemes");
}

static void
wewm_init_supporting_window(void)
{
    ...
    wmcheckwin = XCreateSimpleWindow(dpy, root, 0, 0, 1, 1, 0,
                                     ...
                                     BlackPixel(dpy, screen));
    if (wmcheckwin == None)
        die("wewm: failed to create EWMH supporting window");
    ...
    XChangeProperty(dpy, wmcheckwin, name, utf8, 8, PropModeReplace,
                    (unsigned char *)WEWM_VERSION, (int)strlen(WEWM_VERSION));
}

static void
wewm_init_ewmh_root_properties(void)
{
    Atom supported[] = {
...
static unsigned int
wewm_modifier_variant_count(unsigned int mod, unsigned int variants[4])
{
...
static void
wewm_grab_buttons(Window win, enum ClickRegion region)
{
    size_t i;
    if (!dpy || win == None)
        return;
    ...
    for (i = 0; i < wewm_button_count; i++) {
        ...
        if (b->region != region || b->button == 0u || b->button > 255u)
            continue;
        count = wewm_modifier_variant_count(b->mod, variants);
        for (j = 0; j < count; j++)
...
static void
wewm_grab_client_buttons(Client *c, bool focused)
{
    size_t i;
    if (!c || c->win == None || !dpy)
        return;
...
static void
wewm_grab_root_buttons(void)
{
    wewm_grab_buttons(root, ClickRootBackground);
}

static void
wewm_grab_bar_buttons(Monitor *m)
{
    if (!m || m->barwin == None)
        return;
    wewm_grab_buttons(m->barwin, ClickTagIndicator);
    wewm_grab_buttons(m->barwin, ClickLayoutSymbol);
    wewm_grab_buttons(m->barwin, ClickStatusText);
    wewm_grab_buttons(m->barwin, ClickWindowTitle);
}

static void
wewm_init_status(void)
{
    Atom actual_type;
    int format;
    unsigned long nitems, bytes_after;
    ...
    if (XGetWindowProperty(dpy, root, XA_WM_NAME, 0, WEWM_STATUS_MAX - 1,
                           False, AnyPropertyType, &actual_type, &format,
                           &nitems, &bytes_after, &data) == Success &&
        data && format == 8 && nitems > 0) {
        data[nitems < WEWM_STATUS_MAX ? nitems : WEWM_STATUS_MAX - 1] = '\0';
        wewm_set_status((const char *)data);
    } else {
        wewm_set_status(NULL);
    }
    if (data)
        XFree(data);
}
...
static void
wewm_startup(int argc, char **argv)
{
    wewm_check_args(argc, argv);
    wewm_check_locale();

    dpy = XOpenDisplay(NULL);
    if (!dpy)
        die("wewm: cannot open X display");
    ...
    wewm_claim_root();
    wewm_init_drawing();
    if (wewm_discover_monitors() && !selmon)
        die("wewm: monitor discovery produced no active monitor");
    wewm_init_atoms();
    wewm_init_cursors();
    wewm_init_schemes();
    wewm_init_status();
    wewm_init_supporting_window();
    wewm_init_ewmh_root_properties();
    wewm_init_root_input();
    wewm_update_bars();
    XSync(dpy, False);
    numlockmask = wewm_get_numlock_mask();
    wewm_regrab_keys();
    wewm_grab_root_buttons();
    wewm_populate_dispatch();
    wewm_scan_existing_windows();
    wewm_focus(selmon, NULL);
    wewm_arrange_all();
    XSync(dpy, False);
    running = true;
...
wewm_init_state(void)
{
    ...
    (void)wewm_set_tag_count(WEWM_DEFAULT_TAG_COUNT);
    numlockmask = 0;
    wewm_set_status(NULL);

    /* Keep the built-in action entry points directly linkable even while the
     * configuration tables are intentionally empty. */
    (void)wewm_action_spawn;
    (void)wewm_action_focus_stack;
    (void)wewm_action_inc_nmaster;
    (void)wewm_action_set_mfact;
    (void)wewm_action_zoom;
    (void)wewm_action_cycle_saved;
    (void)wewm_action_toggle_floating;
    (void)wewm_action_toggle_bar;
    (void)wewm_action_view;
    (void)wewm_action_toggle_view;
    (void)wewm_action_tag;
    (void)wewm_action_toggletag;
    (void)wewm_action_focus_monitor;
    (void)wewm_action_send_monitor;
    (void)wewm_action_move_resize;
    (void)wewm_action_kill;
    (void)wewm_action_quit;
